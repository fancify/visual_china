...
## 用户新增需求（2026-05-02 第三轮）
- ⏳ **走近城市可点击展开详情面板**：包括城市历史背景、来源、地理意义；
  比较详细的介绍，可展开 / 可收起。基础设施跟"信息分层 UI"（P5）合并
  做。proximity trigger（玩家距离 <某阈值时浮现 hint），点击展开。

## 用户新增需求（2026-05-02 第二轮）
- ⏳ **城市建筑造型重做** —— 现在 base+roof 两层的样子用户觉得"丑"。
  改成"矮 + 口字型"：单层薄方框 / 围墙 + 中间空（俯视像 "口"），
  no roof prism。配合用户脑中"小城"形象。tier 之间用尺寸差区分。
- ⏳ **高视角依然看不见河** —— polygonOffset 已 -2/-4，仍然在某些
  俯视角度看不见。需要二轮诊断：是不是 ribbon depth 还是被 ground
  pixels 占了？或者 cap lift 在某些 chunk 边界处把 ribbon 推得过低？
- ⏳ **LOD / 距离淡入分层加载** —— 用户感觉同时加载太多导致信息过载
  且性能压力大。要点：
  1. 远处不加载（chunk streaming 已经类似，但城市/POI/水/河也要 LOD）
  2. 渐进淡入（fade 从 0 到 1，**不要** snap 0→1 一蹦出来）
  3. terrain 自身也要 LOD（远处低分辨率、近处高分辨率）
  4. 不只是 mesh visibility，texture / vertex 也跟距离 fade

## 用户新增需求（2026-05-02 第一轮）
