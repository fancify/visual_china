...
export function northNeedleAngleRadians(cameraHeading: number): number {
...
function clockwiseFromNorth(vec: { x: number; z: number }): number {
  // dot(vec, MAP_NORTH) = vec.x * 0 + vec.z * (-1) = -vec.z
  // cross(vec, MAP_NORTH 顺时针轴) = vec.x * 1 + vec.z * 0 = vec.x
  return Math.atan2(vec.x, -vec.z);
}

export function cameraDirectionLabel(cameraHeading: number): string {
  const forward = cameraForwardVector(cameraHeading);
...
export function screenRightDirectionLabel(cameraHeading: number): string {
  const right = cameraRightVector(cameraHeading);
