...
function waterNameMatches(feature: HydrographyFeature, names: string[]): boolean {
  const labels = [feature.name, feature.displayName, ...(feature.aliases ?? [])].filter(Boolean) as string[];
