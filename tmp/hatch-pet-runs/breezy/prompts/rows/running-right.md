Create a single horizontal sprite strip for the Codex app digital pet `breezy` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: friendly feminine chibi human pet, side-parted dark brown hair with one loose front sweep, bright almond eyes, gentle smile, white zip-up jacket with a high collar, compact body, clean silhouette, warm and calm personality.
