...
更新时间：2026-05-10
版本：**v2.1**（v2 收敛后吸收 Codex r5 技术注释：实施细节 + 数据管线 + shader 注入统一）
...
#### 学习产出
- `docs/learning/botw-distance-fade.md` — 记录 geomorphing 的 vertex shader 实现 + 云投影的 alpha modulation 数学
...
#### 季节变化（4 季 cycle，已有 20min/季 切换骨架）

| BotW 怎么做 | 浏览器适配 | 当前差距 |
|---|---|---|
| 地表色调按季节 lerp（春绿/夏深绿/秋黄红/冬枯白） | terrain shader 加 seasonalTint uniform，按 EnvironmentController.season lerp | ✅ 部分有，需强化色彩区分度 |
| 植被颜色按季节变（樱花春开 / 枫叶秋红 / 冬枯枝） | scenery instanced material 接 seasonalTint uniform，按树种切换 | ❌ 当前所有树同色 |
| 落叶飘飞（秋季） | 程序生成 leaf particle，按 WindManager 漂移，地面累积一层 | ❌ 没 |
| 雪覆盖（冬季高海拔） | terrain shader 按 (season=winter) AND (elevation > threshold) 在 fragment shader 加 snow blend | ❌ 没 |
| 水部分结冰（冬季） | water shader 按季节 + 海拔 切到 ice texture + 减反射 | ❌ 没 |
| 桃李梨花点缀（春季） | scenery 春季在某些 chunk 增加 blossom particle / instance | ❌ 没 |
...
| BotW 怎么做 | 浏览器适配 | 当前差距 |
|---|---|---|
| 日出 / 日落更剧烈的 palette shift | sky shader 在 sun altitude 临界值（-5° 到 +15°）做 dramatic palette ramp | ⚠️ 当前过渡平淡 |
| 山体边缘金边（晨昏 rim 强化） | rim shader uniform 接 sun altitude，晨昏时 rim strength ×3 | ❌ 没（依赖 Phase 2 cel + rim） |
| 夏夜萤火虫 | 玩家附近 5-15u 范围生成飘动 sprite particle，仅夏季夜晚 | ❌ 没 |
| 远城灯火点点（夜晚） | 城墙 instance 夜晚切到 emissive 黄光（已有部分）+ 远距离仍可见的小光点 | ⚠️ 当前夜晚城市偏暗 |
| 月光照明（满月时增强） | moonLight 强度按月相调节，满月时 ambient ×1.5 | ❌ 当前固定 |
...
| BotW 怎么做 | 浏览器适配 | 当前差距 |
|---|---|---|
| 天气状态间平滑 lerp（不硬切） | weather lerp over 8-15s，所有 uniform 同时插值 | ⚠️ 当前是 instant 切 |
| 多云 / 阴天 variant | layered cloud planes 加密 + ambient 降饱和 | ❌ 没（todo 已列） |
| 晨雾（早晨 5-7am 自动起雾） | volumetric height fog 早晨 density ×2，10am 散去 | ❌ 没 |
| 山间雾（低洼区 + 早晨） | height-based fog uniform 在 elevation < threshold 区域加 density | ❌ 没 |
| 雨后地面湿润 reflection | 雨后 30s 内 terrain fragment 加 wetness factor，水分残留反射 | ❌ 没 |
| 雪后雪盖累积（一段时间） | 雪天持续后地面 snow blend factor 渐增 | ❌ 没 |

**预算**：+0.5ms（多云 layered cloud 增量 + height fog + wetness）

**工程量**：2 天

#### 动植物变化（已有 wildlife 系统）

| BotW 怎么做 | 浏览器适配 | 当前差距 |
|---|---|---|
| 候鸟季节性迁徙（春北/秋南） | 春秋季节在天空生成飞鸟 V 字队列，按方向飞过 | ❌ 没 |
| 季节性鸣叫（夏蝉鸣 / 秋虫鸣 / 冬寂静） | audio mixer 按季节切环境音 sample | ⚠️ 部分有，需强化 |
| 动物季节 visibility（冬季罕见 / 春季活跃） | wildlife spawn rate 按季节调节 | ❌ 当前固定 |
...
| 子任务 | 来源 | 工程量 |
|---|---|---|
| HUD mini-map 节流 0.15s → 0.5s + features cache | `performance-baseline.md` | 0.5 天 |
| GroundAnchorRegistry 池 + chunk reanchor | r1 B + Codex r2 修订 | 1 天 |
| 城市/路径/scenery anchor 接管 + 路径 per-vertex sample | r1 B 扩展 | 1 天 |
| 4 档 LOD geomorphing | r1 C + 1.1 BotW 学习 | 1.5 天 |
| Distance impostor (scenery) | r2 I | 0.5 天 |
| Atmospheric haze + 云投影地表 | 1.1 BotW 学习 | 1 天 |
| `lastHitChunk` 缓存 | r2 优化 | 0.25 天 |
| WindManager / EnvironmentUniforms 全局 | Codex r3 #6 | 0.25 天 |
...
### Phase 6 — 世界的呼吸（季节 / 时间 / 天气 / 动植物 / 地域 biome） (8-9 天)
...
| **地域 biome 数据收集**（秦岭/关中/汉中/四川 4 区代表性 flora + fauna 各 3-5 种，IUCN/中国植物志查证） | 1.4.7 地域 + 用户钉死 | 2 天 |
...
| **Phase 6 世界呼吸+地域 biome（12 区物种）** | 6-7 | 8-9 | **12-16 天** | **44-57 天** |

**v2.1 总工程：44-57 天**（v1 29-30 天 → +50-90%，吸收 r4 + r5 两轮 critique）。
...
- 估时再校准（v2 → v2.1）：Phase 0 8-10 → 12-15；Phase 6 8-9 → 12-16（**总 v2.1 估时 40-52 天**）✅
- 关键盲点（已新增到 v2.1）：
...
## 7.1 数据管线 asset schema 契约（r5 关键盲点 — 必须 Phase 0 第 0 步定义）
