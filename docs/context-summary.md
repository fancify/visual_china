...
- 现代水系：已建立独立 hydrography 事实层、秦岭 curated skeleton、DEM mismatch report，以及 OSM/Overpass 命名水系导入管线；当前 OSM 现代水系资产覆盖 4639 条命名水系片段，并以缩放分级方式接入 Atlas Workbench。
...
- 汉中入蜀路线：金牛道/剑门蜀道。
- 显式水系骨架：渭河、汉水、嘉陵江、褒水、斜水，已从 Atlas 硬编码迁到 hydrography asset 派生；OSM 导入层补充主要河流、命名支流和渠道。
- 2D 地貌总览可从 Atlas 绘制核心地貌、水系、古道、城市、关隘；全屏地图会按缩放逐级显示 OSM 水系证据层。
...
- 水系已接入 OSM/Overpass 命名水系导入，但仍需继续做主干/支流等级校准、名称核对、断裂线合并，以及与 DEM/河谷视觉贴合。
